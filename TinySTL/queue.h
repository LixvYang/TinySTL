...
namespace tinystl
{
  ...
  // 模板类 priority_queue
  // 参数一代表数据类型，参数二代表容器类型，缺省使用 tinystl::vector 作为底层容器
  ...
  public:
    ...
    priority_queue(priority_queue &&rhs)
        : c_(tinystl::move(rhs.c_)), comp_(rhs.comp_)
    {
      tinystl::make_heap(c_.begin(), c_.end(), comp_);
    }
  ...
  public:
    ...
    void swap(priority_queue &rhs) noexcept(noexcept(tinystl::swap(c_, rhs.c_)) &&noexcept(tinystl::swap(comp_, rhs.comp_)))
    {
      tinystl::swap(c_, rhs.c_);
